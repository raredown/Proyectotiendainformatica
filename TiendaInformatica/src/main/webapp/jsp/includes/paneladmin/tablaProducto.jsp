...
<html>
    ...
    <table border="1" align="center" id="tblDatos" class="table table-striped">
        ...
        <c:forEach var="i" items="${requestScope.todoproductos}">
            <tr>
                <td><c:out value="${i.denominacion}"/></td>
                <td><input type="number" id="stock<c:out value="${i.idProducto}"/>" value="<c:out value="${i.stock}"/>" onchange="updatestock('<c:out value="${i.idProducto}"/>','stock<c:out value="${i.idProducto}"/>')"></td>
                <td><c:out value="${i.stockMinimo}"/></td>
                <c:if test="${i.fueraCatalogo=='n'}">
                    <td>
                        <input id="<c:out value="${i.idProducto}"/>" type="checkbox" name="bloquear" value="" onclick="updatebloquearProducto('<c:out value="${i.idProducto}"/>')">
                    </td>
                </c:if>
                <c:if test="${i.fueraCatalogo=='s'}">
                    <td>
                        <input id="<c:out value="${i.idProducto}"/>" type="checkbox" name="bloquear" value="" checked onclick="updatebloquearProducto('<c:out value="${i.idProducto}"/>')">
                    </td>
                </c:if>
